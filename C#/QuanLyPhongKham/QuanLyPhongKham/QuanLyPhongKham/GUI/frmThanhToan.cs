...
            }
        ...
        private void LoadBangThuoc(int maPhieu)
        {
            ...
            if (dgvThuoc.Columns.Count > 0)
            {
                ...
                // --- PHẦN MỚI THÊM ---
                if (dgvThuoc.Columns.Contains("ThanhTien"))
                {
                    dgvThuoc.Columns["ThanhTien"].HeaderText = "Thành Tiền";
                    dgvThuoc.Columns["ThanhTien"].DefaultCellStyle.Format = "N0"; // Định dạng số có dấu phẩy
        ...
        private void LoadBangDichVu(int maPhieu)
        {
            ...
            if (dgvDichVu.Columns.Count > 0)
            {
                ...
                if (dgvDichVu.Columns.Contains("ThanhTien"))
                {
                    ...
                    dgvDichVu.Columns["ThanhTien"].DefaultCellStyle.Format = "N0"; // Định dạng số có dấu phẩy
                    dgvDichVu.Columns["ThanhTien"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                }
            }
        }
