...
namespace QuanLyPhongKham.DAL
{
    public class PhieuKhamDAL
    {
        ...
        // Trong file PhieuKhamDAL.cs

        // [SỬA] Thêm tham số string ketQuaDV
        public bool InsertChiTietDV(int maPhieu, int maDV, int soLuong, decimal donGia, decimal thanhTien, string ketQuaDV)
        {
            // Thêm KetQuaDV vào câu lệnh INSERT
            string query = @"INSERT INTO CHITIETDICHVU (MaPhieu, MaDV, SoLuongDV, DonGiaDV, ThanhTienDV, KetQuaDV)
                     VALUES (@MaPhieu, @MaDV, @SL, @Gia, @ThanhTien, @KetQua)";
            ...
            SqlParameter[] param = new SqlParameter[]
            {
                ...
                // [MỚI] Thêm tham số kết quả (xử lý null nếu không nhập)
                new SqlParameter("@KetQua", ketQuaDV ?? (object)DBNull.Value)
            };
            return DatabaseHelper.ExecuteNonQuery(query, param) > 0;
        }
